...
import { Search, ArrowUpDown, ChevronUp, ChevronDown, FileDown, FilePen, Edit2, Trash, UserCircle, LayoutGrid, Eye } from 'lucide-react';
          ...
          <table className="w-full text-left border-collapse table-auto min-w-[640px]">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700">
                ...
                <th
                  ...
                  className="px-2 sm:px-3 py-4 sm:py-6 text-[10px] sm:text-[11px] font-black uppercase tracking-wider sm:tracking-[0.2em] text-slate-400 dark:text-slate-500 cursor-pointer hover:bg-white dark:hover:bg-slate-800 transition-colors group select-none hidden md:table-cell"
                >
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/50">
              {paginatedData.map(record => (
                <tr key={record.id} className="hover:bg-indigo-50/20 dark:hover:bg-indigo-900/10 transition-all group/row">
                  ...
                  <td className="pl-2 sm:pl-3 pr-4 sm:pr-8 py-4 sm:py-5 text-right">
                    ...
                    <div className="hidden lg:flex gap-1.5 justify-end">
                      <button
                        onClick={() => setPreviewRecord(record)}
                        className="p-2 sm:p-2.5 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-xl hover:bg-purple-600 hover:text-white transition-all shadow-sm active:scale-90"
                        title="Previsualizar"
                      >
                        <Eye size={14} />
                      </button>
                      <button
                        onClick={() => handleGeneratePDF(record, true)}
                        className="p-2 sm:p-2.5 bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded-xl hover:bg-rose-600 hover:text-white transition-all shadow-sm active:scale-90"
                        title="Ver PDF"
                      >
                        <FileDown size={14} />
                      </button>
                      <button
                        onClick={() => handleGeneratePDF(record, false)}
                        className="p-2 sm:p-2.5 bg-sky-50 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400 rounded-xl hover:bg-sky-600 hover:text-white transition-all shadow-sm active:scale-90"
                      ...
                      <button
                        ...
                        className="p-2 sm:p-2.5 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-xl hover:bg-amber-500 hover:text-white transition-all shadow-sm active:scale-90"
                        title="Modificar"
                      >
                        <Edit2 size={14} />
                      </button>
                      <button
                        ...
                        className="p-2 sm:p-2.5 bg-red-50 dark:bg-red-900/30 text-red-500 dark:text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-sm active:scale-90"
                        title="Eliminar"
                      >
                        <Trash size={14} />
                      </button>
              ...
              {paginatedData.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-6 sm:px-10 py-16 sm:py-24 text-center">
